...
class Music(commands.Cog):
    ...
    async def play(self, ctx, *, arg : str = None):
        ...
        if arg is None and voice_client.is_playing():
            voice_client.pause()
            em = discord.Embed(title=":musical_note: **Song paused** :musical_note:", colour=discord.Color.from_rgb(255,105,180)())
            em.set_footer(text=f"Requested by {ctx.author.name}", icon_url=ctx.author.avatar_url)
    ...
    #     if ctx.guild.id in self.loop.keys():
